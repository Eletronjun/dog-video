...
function Passeadores() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:3001'}/passeadores`);
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:3001'}/passeadores/${selectedPasseador.id}`, {
        method: 'DELETE',
      });
