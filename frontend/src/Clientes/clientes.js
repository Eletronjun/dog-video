...
function Clientes() {
  ...
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_API_URL || 'http://localhost:3001'}/clientes`)
      .then(response => {
  ...
  const handleDelete = async () => {
    try {
      const response = await axios.delete(`${process.env.REACT_APP_API_URL || 'http://localhost:3001'}/clientes/${selectedCliente.id_cliente}`);
